    ...
    <div class="wrapper">
      <div class="house-detail">
        ...
        <div class="title-information">
          <div class="title-item" style="paddingRight:2.16rem">
            <div class="item-top">成交价格</div>
            <div class="item-bottom">{{houseInfo.trade_amount_w}} 万</div>

          </div>
          <div class="title-item" style="paddingLeft:0.6rem">
            <div class="item-top">单价</div>
            <div class="item-bottom">{{houseInfo.average_price}}元/㎡</div>
          </div>
        </div>
        ...
        <div class="specific-information">
          ...
          <div class="specific-box">
            ...
            <div class="box-item">
              楼型：<span>{{houseInfo.building_type_name}}</span>
            </div>
          </div>
          <div class="specific-box">
            <div class="box-item box-item-left">
          ...
          <div class="specific-box">
            <div class="box-item box-item-left">
              产权：<span>{{houseInfo.property_year_name}}</span>
            </div>
            <div class="box-item">
              用途：<span>{{houseInfo.purpose_name}}</span>
            </div>
          </div>
          <div class="specific-box">
            <div class="box-item box-item-left">
              权属：<span>{{houseInfo.ownership_name}}</span>
            </div>
          </div>
        </div>
        ...
        <div class="split-line"></div>
        <div class="recommended-house" v-if="saleRecommend.length && !deaLock">
          <div class="recommended-title">推荐二手房</div>
          <houselistItem :list="saleRecommend" @goDetail="goSecondRecommend"></houselistItem>
          <div class="morehouse-btn" @click="toolsGoPageView({path:'/home/list/secondhouseList', query: {isUpdate: 1,uptown_id:houseInfo.uptown_id}})">更多二手房</div>
        </div>
        <div class="split-line"></div>
...
export default {
  ...
  async mounted () {
    window.scroll(0, 0);
    setTimeout(async () => {
      this.shouseTradeId = this.$route.query.shouse_trade_id || ''
      //初始化登陆的状态
      await this.init();
      this.getVillageDeal()
  ...
  methods: {
    ...
    ...mapActions("home", ["rentDetail", "getAppFangDealShouseDetail", "getAppWechatNearbyGetRecommendBroker"]),
    // 初始化
    async init () {
    ...
    async getDetail () {
      const postData = {
        shouse_trade_id: this.shouseTradeId || ''
      }
      let res = await this.$store.dispatch("home/getAppFangDealShouseDetail", postData);
      if (res) {
    ...
    goNewMapPosition () {
      ...
      param.houseId = this.houseInfo.shouse_id;
      param.houseType = 4;
      ...
      param.price_name = this.houseInfo.trade_amount_w + '万'
      // 跳转经纪人聊天所需参数
      ...
      param.average_price_name = this.houseInfo.average_price + '元/m²'
      param.price = this.houseInfo.trade_amount
      param.acreage_cover = this.houseInfo.acreage_cover
      param.shouse_trade_id = this.shouseTradeId // 成交详情id
    ...
    goChat (brokerInfo, type) {
      ...
      let houseInfo = {
        // 这里写 房源消息的内容
        shouse_id: this.shouseTradeId,//->  二手房id, 应该是pc的房源id
        shouse_title: this.headTitle,//->  标题
        preview_url: this.$appConfig.appWebStaticUrl + 'common/house.png',//->  图片地址
        ...
        unit_price: this.houseInfo.average_price,//->  单价
        price: this.houseInfo.trade_amount,//->  售价
        acreage_cover: this.houseInfo.acreage_cover,//->  建筑面积
    ...
    async handleShare (type = 0) {
      ...
      const shareData = await this.getShareApp({
        ...
        jsonData: {
          provider: "weixin",
          title: `【${this.houseInfo.district_name}${this.houseInfo.block_name}】${this.headTitle} ${this.houseInfo.trade_amount_w + '万'}`,
    ...
    async getAppFangSalelistRecommend () {
      const postData = {
        id: this.houseInfo.shouse_id,//	否	string	二手房id
    ...
    async callPhone (val) {
      try {
        const postData = {
          ...
          ex2: this.houseInfo.shouse_id || "",//否	string	房源id
        }
  ...
  .wrapper {
    .house-detail {
      .title {
        ...
        color: #303030;
        line-height: 0.56rem;
        padding: 0.2rem 0.54rem 0.42rem 0.46rem;
      }
      .title-information {
        display: flex;
