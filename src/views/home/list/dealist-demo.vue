...
export default {
  ...
  async activated () {
    if (!this.$route.meta.canKeep) {
      // this.houselist = []
      const toggleArray = ["district", "huxing"];
      // 避免获取不到dom
      this.isClosed = true;
      setTimeout(() => {
  ...
  methods: {
    ...
    async getEchartData () {
      const postData = {
        uptown_id: this.searchPostGetData.searchUptownId || "",//	否	string	小区id
      }
      let res = await this.$store.dispatch('home/getAppFangcDealaUptownPrice', postData);
      if (res) {
        res.forEach(item => {
        ...
        console.log('最大', this.priceMin, this.priceMax);
            ...
            series: [
              {
                ...
                label: {
                  ...
                  position: 'top',
                  formatter: params => {
                    console.log('什么哇', params);
                    if (params.dataIndex == this.uptownPriceList.length - 1) {
                      return '{active|' + params.value[1] + 'w}'
                    }
                    return params.value[1] + 'w'
                  },
                  rich: {
                    active: {
                      color: '#FFFFFF',
    ...
    async getListConfigInit () {
      ...
      if (!this.$commonUtils.utilCore.empty(this.searchPostGetData) && this.pageFrom !== "common-index") {
        ...
        if (this.pageFrom === "home-detail-searchdeal") {
          ...
          this.postGetData.leaseSaleIndex = this.searchPostGetData.leaseSaleIndex || '1'
        }
        // 都是从详情页的成交同户型进来的 
    ...
    handleDropdownClone (type = 0) {
      ...
      const cloneArray = ["huxing", "district", "addresslist", "districtMetro", "addresslistMetro", "tabPriceIndex", "tabAreaIndex", "postGetData"];
...
.list {
  .fixed-container {
    ...
    .header {
      padding: 0.2rem 0.74rem 0 0.4rem;
      display: flex;
      align-items: center;
