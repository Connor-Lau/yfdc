  ...
  <div class="list">
    ...
    <div class="content-header" :style="{'padding-top':navheight+'px'}">
      <van-list @load="actOnLoad"
        ...
        finished-text="没有更多了">
        <houselistItem :list="houselist" @goDetail="goDetail"></houselistItem>
      </van-list>
    </div>
    <!-- 回到顶部 -->
    <scrollTop></scrollTop>
...
export default {
  ...
  computed: {
    ...
    huxingClass () {//是否有选择户型里的条件
      if (!this.$commonUtils.utilCore.empty(this.postGetData.setHuxing) && !this.$commonUtils.utilCore.empty(this.postGetData.setHuxing[0].id)) {
        return "choose-active"
      }
      return ""
    },
  ...
  methods: {
    ...
    async actOnLoad () {
      ...
      let res = await this.$store.dispatch("home/actAppFangSaleLists", this.postGetData);
      this.postGetData.loading = false;
      if (res && res.rows) {
    ...
    async getListConfigInit () {
      ...
      //二级标签-二手房
      res = await this.$store.dispatch('home/getAppFangSaleGetSecondTag');
      if (res) {
        // this.$commonUtils.appLog.log("返回结果", res)
        this.diskSourceType = []
    ...
    async handleType () {
      ...
      this.postGetData.city_id = this.$route.query.city_id || this.switchCityInfo.city_id
      //把数据存在vuex里去
      await this.$store.commit('home/setSearchPostGetData', this.postGetData);
      //清除当前列表数据
      this.houselist = []
    ...
    async closeDropdownMenu (type, isClose = true) {
      ...
      this.houselist = []
      await this.actOnLoad();
      console.log('已选参数', this.postGetData);
