  ...
  <div class="list">
    <div class="fixed-container">
      ...
      <div class="header">
        ...
        <div class="search">
          <input type="text" v-model="postGetData.searchKeyWord" placeholder="请输入楼盘名称查找" @click="getSearch()">
      ...
      <div class="search-condition">
        <van-dropdown-menu active-color="#000000" :close-on-click-outside="false">
          ...
          <van-dropdown-item title="价格" ref="priceTab" :title-class="priceTabClass" @opened="handleDropdownClone(1)" @close="handleDropdownClone()">
            <div class="house-price house-type">
              <div class="price-tab">
                ...
                <div class="tab-second" @click="selectTabArea(1,'priceTab')" :class="tabPriceIndex == 1?'price-tab-active':''">单价 (万/㎡)</div>
              </div>
              <template v-if="tabPriceIndex == 0">
                <div class="type-content price-content">
                  <div class="type-item" v-for="(item,index) in priceTabFirst" :key="index" @click="chooseDistrictAddress(item,'priceTab')" :class="checkArrayValue(postGetData.setPriceTabFirst,'id',item.id) != -1?'type-active':''">{{item.name}}</div>
                </div>
                <div class="price-ipt">
                  <input type="number" class="_iosKeyboardInput" v-model="postGetData.setPriceFirstVal[0]" placeholder="最低总价" @keyup="checkPriceInput(0)">
                  <span>至</span>
                  <input type="number" class="_iosKeyboardInput" v-model="postGetData.setPriceFirstVal[1]" placeholder="最高总价" @keyup="checkPriceInput(1)">
                </div>
              </template>
              <template v-if="tabPriceIndex == 1">
                ...
                <div class="price-ipt">
                  <input type="number" class="_iosKeyboardInput" v-model="postGetData.setPriceSecondVal[0]" placeholder="最低单价" @keyup="checkPriceInput(0)">
                  <span>至</span>
                  <input type="number" class="_iosKeyboardInput" v-model="postGetData.setPriceSecondVal[1]" placeholder="最高单价" @keyup="checkPriceInput(1)">
                </div>
              </template>
              <div class="reset-btn">
                <div class="btn-box">
              ...
              <div class="more-content">
                <div class="more-left">
                  <ul>
                    ...
                    <!-- <li @click="selectTabArea(1,'moreTitle')" :class="moreTitle == 1 ? 'more-active':''">物业类型</li> -->
                    <!-- 后端接口说新房没有装修请况这个字段的-->
    ...
    <div class="content-header" :style="{'padding-top':navheight+'px'}">
      <van-list @load="actOnLoad"
        ...
        finished-text="没有更多了">
        <newhouselistItem :list="houselist" @goDetail="goDetail" :isNewHouse="true"></newhouselistItem>
      </van-list>
    </div>
    <scrollTop></scrollTop>
...
export default {
  ...
  methods: {
    ...
    async actOnLoad () {
      ...
      this.postGetData.city_id = this.switchCityInfo.city_id
      this.postGetData.loading = true;
      let res = await this.$store.dispatch("home/actAppFangHouseLists", this.postGetData);
      this.postGetData.loading = false;
      if (res && res.rows) {
    ...
    async getListConfigInit () {
      ...
      if (!this.$commonUtils.utilCore.empty(this.searchPostGetData) && (this.pageFrom !== "common-index") && this.isUpdate != 1) {
        ...
        if (this.pageFrom === "home-newsearch") {
      ...
      //价格区间列表（新房二手房）
      res = await this.$store.dispatch('home/getAppBase_shouse_total_price');
      if (res) {
        //this.$commonUtils.appLog.log("返回结果", res)
        this.priceTabFirst = []
        for (let key in res) {
      ...
      //物业类型-新房
      res = await this.$store.dispatch('home/getAppBase_property_type');
      if (res) {
    ...
    checkInputTwoVal (type) {
      ...
      if (type == 'priceTab' &&
        ...
        if (this.postGetData.setPriceSecondVal[0] > Number(this.postGetData.setPriceSecondVal[1]) || Number(this.postGetData.setPriceSecondVal[1] == '0')) {
