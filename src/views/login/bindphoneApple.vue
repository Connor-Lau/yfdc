...
export default {
  ...
  computed: {
    ...mapState("user", ["userToken", "userInfo", "isLogin", "preLoginAppleVal"]), //用户Token,用户登陆信息
  },
  async mounted () {
    //如果没有苹果预登陆的数据，则跳回苹果登陆页
    if (this.$commonUtils.utilCore.empty(this.preLoginAppleVal)) {
  ...
  methods: {
    ...
    async actBindphone () {
      //如果没有进行苹果预登陆，则报错
      if (
        this.$commonUtils.utilCore.empty(this.preLoginAppleVal)
        || this.$commonUtils.utilCore.empty(this.preLoginAppleVal.userInfo)
        || this.$commonUtils.utilCore.empty(this.preLoginAppleVal.userInfo.openId)
      ) {

        this.$commonUtils.appLog.log("登陆有出错信息", this.preLoginAppleVal)

        this.$commonUtils.utilCore.showToast("苹果登录出错！");
        setTimeout(() => {
          this.toolsGoPageView({ path: '/login/appleLogin' })
        }, 3000)
        return
      }
      //如果手机号不正确
      ...
      try {
        let postData = {
          ...
          apple_open_id: this.preLoginAppleVal.userInfo.openId
        }
        ...
        if (tmp_info) {//登陆成功，跳到我的页面
          ...
          this.$commonUtils.utilCore.showToast("苹果绑定手机号出错");
        }
      } catch (err) {
